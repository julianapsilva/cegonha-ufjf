...
const ListCoverAdress = () => {
  ...
  const handleDelete = (id, index) => {
    const confirm = window.confirm(
      `Deseja realmente deletar endereço coberto?`
    );
    if (confirm) {
      api
        .delete("cover-address/" + id)
        .then(
          data.splice(index, 1),
          setData([...data]),
        ...
        <table>
          <thead>
            <tr className="title">
              ...
              <th>CEP</th>
              <th>Bairro</th>
              <th className="cover-adress-city-uf">Cidade-UF</th>
              <th>Editar</th>
              <th>Excluir</th>
            </tr>
          </thead>
          <tbody>
            {data.map(
              (
                ...
                <tr key={id}>
                  <td>
                    <div className="city-mobile-version">{street}</div>
                  </td>
                  <td>
                    {number_start} ao {number_end}
                  </td>
                  <td>{cep}</td>
                  <td>{district}</td>
                  <td className="cover-adress-city-uf">
                    {city}, {uf}
                  </td>
                  <td>
                    <button
